    ...
    while True:
        for server in SERVERS:
            ...
            if is_healthy and server in failed_servers:
                ...
                logger.info(f"[RECOVERED] Health check: Servidor {server} recuperado y vuelve a estar activo")
            elif not is_healthy and server not in failed_servers:
                # Servidor caído
                failed_servers[server] = time.time()
                logger.warning(f"[DOWN] Health check: Servidor {server} detectado como caído")
...
def proxy(path):
    ...
    for server in active_servers:
        ...
        try:
            ...
            logger.info(f"[OK] Solicitud exitosa a: {url}")

            # Si el servidor estaba marcado como caído, quitarlo de la lista
            if server in failed_servers:
                del failed_servers[server]
                logger.info(f"[RECOVERED] Servidor {server} recuperado y vuelve a estar activo")
            ...
            last_error = e
            logger.error(f"[DOWN] Error al conectar con {server}: {str(e)}")
            failed_servers[server] = time.time()
...
def server_status():
    ...
    for server, info in status.items():
        if info["status"] == "UP":
            html += f'<div class="server up">[OK] {server}: ACTIVO</div>'
        else:
            html += f'<div class="server down">[DOWN] {server}: CAÍDO (por {info["downtime_seconds"]} segundos, reintento en {info["retry_in"]} segundos)</div>'
...
def check_servers_on_startup():
    ...
    for server in SERVERS:
        if check_server_health(server):
            logger.info(f"[OK] Servidor {server} activo")
        else:
            failed_servers[server] = time.time()
            logger.warning(f"[DOWN] Servidor {server} no responde al inicio")
